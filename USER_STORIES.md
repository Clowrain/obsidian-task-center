...
- `US-101` 一眼看到**本周 7 天**谁塞满了,今日高亮。切到上一周、下一周。桌面 / 大屏 week 是 7 个日期列；移动端 week 是 7 个日期 row（见 US-503）。两者只是布局不同,**日期归属、任务计数、估时合计、改期结果必须完全一致**。周视图主区域不能塌得太矮：桌面与移动端的 week 主体最小高度至少占当前 Task Center 可视高度的一半,否则 PM 认为视觉上不成立。
...
- `US-114` 桌面拖拽过程中悬停在另一个 tab 上一会儿，自动切到那个 tab，松手即可跨视图改期。移动端不提供拖拽 / dwell，跨视图动作走 US-507 的显式操作路径。
...
- `US-121` **桌面周视图拖拽改期**：把卡从"周三"拖到"周五",这张卡的 `⏳` 就改成周五。更完整地说,任何任务卡落到 week 的某个**日期目标**上,就把该任务自己的 `⏳` 写成那个日期；原来没有 `⏳`（例如来自未排期池）就新增,原来已有 `⏳` 就替换。拖回原本同一天是 no-op：不写文件、不弹 toast、不进 undo 栈。只改被拖任务自己的 `⏳`,不改标题、tag、估时、完成状态、其它 Obsidian Tasks emoji / inline fields。移动端等价改期走 US-507。
  - 桌面 week 的日期目标 = 某天列头、列体空白处、卡片之间的列内空隙。
  - **卡片本体不是日期目标**：如果落点在另一张卡的卡体上,走 US-125 嵌套,不走 US-121 改期。
- `US-122` 桌面月视图每个日期格都可以当拖入目标。月视图日期格的改期语义与 US-121 相同：落到日期格背景 / 日期数字 / `+N more` 等日期表面时写入该日期；落到某张任务卡本体时仍按 US-125 嵌套。移动端月视图不提供拖入目标,点日期格只打开该日列表（US-504）。
...
- `US-503` 周视图在移动端默认垂直 list（7 行折叠面板）；当前日默认展开，其余折叠；row header 显 `星期 MM-DD · N tasks · XhYm`，点击 row header 只负责展开 / 收起。移动端 week 是桌面 7 列 week 的同语义改排版：日期顺序、周边界、任务归属、计数、估时都按 US-101 / US-112 / US-116 执行；改期通过 US-507 的显式动作完成，不把 row header、折叠 row、空白处或卡片缝隙做成 drop target。移动端 week 主体也必须遵守 US-101 的半屏最小高度,避免只露出一条很矮的列表。
